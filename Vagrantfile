...
Vagrant.configure(2) do |config|
  ...
  config.vm.provision "shell", inline: "set -x; sudo sed -i 's|http://http.us|http://ftp.de|g' /etc/apt/sources.list; sudo apt-get update; sudo apt-get install -y pkg-config python2.7-dev"
