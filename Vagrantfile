...
# vagrant ssh
# composer install
# vendor/bin/install DE      or     vendor/bin/install (for all stores, takes some time to finish)
#
# Troubleshooting:
...
['', '-test'].each do |host_suffix|
  ...
  STORES.each do |store|
    HOSTS.push [
      ...
      "gluestorefront#{host_suffix}.#{store}.#{domain}",
      "gluebackend#{host_suffix}.#{store}.#{domain}",
